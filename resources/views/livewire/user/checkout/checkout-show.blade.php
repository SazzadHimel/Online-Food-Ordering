<div class="py-3 py-md-4 checkout">
    <div class="container">
        @if (session('message'))
            <h5 class="alert alert-success">{{session('message')}}</h5>
        @endif
        <h4>Checkout</h4>
        <hr>

        @if ($this->totalProductAmount != '0')
        <div class="row">
            <div class="col-md-12 mb-4">
                <div class="shadow bg-white p-3">
                    <h4 class="text-primary">
                        Total Cost:
                        <span class="float-end">${{$this->totalProductAmount}}</span>
                    </h4>
                    <hr>
                    <small>* Items will be delivered in 30-45 minutes.</small>
                    <br/>
                    <small>* Tax and other charges are included.</small>
                </div>
            </div>
            <div class="col-md-12">
                <div class="shadow bg-white p-3">
                    <h4 class="text-primary">
                        Customer Information
                    </h4>
                    <hr>
                    
                    <div class="row">
                        <div class="col-md-6 mb-3">
                            <label>Full Name</label>
                            <input type="text" wire:model.defer="fullname" class="form-control" placeholder="Enter Full Name" />
                            @error('fullname') <small class='text-danger'></small> @enderror
                        </div>
                        <div class="col-md-6 mb-3">
                            <label>Phone Number</label>
                            <input type="number" wire:model.defer="phone" class="form-control" placeholder="Enter Phone Number" />
                            @error('phone') <small class='text-danger'></small> @enderror
                        </div>
                        <div class="col-md-6 mb-3">
                            <label>Email Address</label>
                            <input type="email" wire:model.defer="email" class="form-control" placeholder="Enter Email Address" />
                            @error('email') <small class='text-danger'></small> @enderror
                        </div>
                        
                        <div class="col-md-12 mb-3">
                            <label>Shipping Address</label>
                            <textarea wire:model.defer="address" class="form-control" rows="2"></textarea>
                            @error('address') <small class='text-danger'></small> @enderror
                        </div>
                        <div class="col-md-12 mb-3">
                            <label>Select Payment Mode: </label>
                            <hr>
                            <div class="d-md-flex align-items-start"></div>
                                <div class="tab-content col-md-9" id="v-pills-tabContent">
                                    <div class="tab-pane active show fade" id="cashOnDeliveryTab" role="tabpanel" aria-labelledby="cashOnDeliveryTab-tab" tabindex="0">
                                        <h6>Cash on Delivery:  
                                            <button type="button" wire:click='codOrder' class="btn w-50 btn-sm">
                                                <span wire:loading.remove wire:target = 'codOrder'>
                                                Place Order (Cash On Delivery)
                                                </span>
                                                <span wire:loading wire:target = 'codOrder'>
                                                Placing Order (Cash On Delivery)
                                                </span>
                                            </button>
                                        </h6>
                                    </div>
                                </div>
                            </div>
                        </div>
                    </div>
                    
                </div>
            </div>
        </div>
        @else
            <div class="card card-body shadow text-center p-md-5">
                <h4>Order Placed Successfully</h4><br>
                <a href="{{ url('/collections') }}" class='long-main-btn btn-warning'>Order More</a>

            </div>
        @endif
    </div>
</div>
